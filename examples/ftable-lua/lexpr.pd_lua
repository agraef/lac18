...
-- AG: some functional programming goodies
require("fp")
...
function lexpr:initialize(sel, atoms)
  ...
  function self:in_1_bang()
    ...
    for i = self.outlets,1,-1 do
      ...
      elseif type(r[i]) == "string" then
        self:outlet(i, "symbol", { r[i] })
      elseif type(r[i]) == "table" then
        self:outlet(i, "list", r[i])
      else
        self:error("calculated a " .. type(r[i]) .. " but expected a number, string or list")
      end
    end
  end
