-- AG: This is exactly like ltabfill in the pd-lua examples, but also outputs
-- a bang to the table receiver after updating the table, so that other
-- objects can respond accordingly.

local waveform = pd.Class:new():register("waveform")
...
function waveform:readexpr(atoms)
  local vname = { }
  local context = { }
  local expr
  local i
  ...
  local f
  local phase = "table"
  for k,v in pairs(waveform_globals) do
    context[k] = v
  end
  for i,v in ipairs(atoms) do
    if phase == "table" then
      if type(v) == "string" then
        ...
        self:error("waveform: table name must be a symbol")
        return -1
      end
      ...
      if v == "->" then
        ...
        if type(v) == "string" then
          ...
          self:error("waveform: variable names must be symbols")
          return -1
        end
      end
      ...
      self:error("waveform: internal error parsing expression")
      return -1
    end end end
  end
...
function waveform:initialize(sel, atoms)
  self.tabname = nil
  self.vname = { }
  self.context = { }
  self.hot = { }
  ...
  function self:in_1_waveform(atoms)
    local inlets
    local vname
    local context
    local f
  ...
  if self.inlets < 1 then
    pd.post("waveform: error: would have no inlets")
    return false
  end
